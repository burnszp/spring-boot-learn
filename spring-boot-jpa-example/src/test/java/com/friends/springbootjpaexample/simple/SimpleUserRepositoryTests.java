 ...
 */
package com.friends.springbootjpaexample.simple;
...
public class SimpleUserRepositoryTests {
	...
	//使用CrudRepository自带方法处理存储和查找
	@Test
	public void findSavedUserById() {

		user = repository.save(user);

		Assertions.assertThat(repository.findById(user.getId())).hasValue(user);
	}

	//使用方法名称构建查询，更多见https://docs.spring.io/spring-data/jpa/docs/2.1.6.RELEASE/reference/html/#repositories.query-methods.query-creation
	@Test
	public void findSavedUserByLastname() throws Exception {

		user = repository.save(user);

		Assertions.assertThat(repository.findByLastname("lastname")).contains(user);
	}
	...
	public void findByFirstnameOrLastname() throws Exception {
		...
		Assertions.assertThat(repository.findByFirstnameOrLastname("lastname")).contains(user);
	}
	...
	public void useOptionalAsReturnAndParameterType() {
		...
		Assertions.assertThat(repository.findByUsername(Optional.of("foobar"))).isEmpty();

		repository.save(user);

		Assertions.assertThat(repository.findByUsername(Optional.of("foobar"))).isPresent();
	}
	...
	public void removeByLastname() {
		...
		Assertions.assertThat(repository.removeByLastname(user.getLastname())).isEqualTo(2L);
		Assertions.assertThat(repository.existsById(user3.getId())).isTrue();
	}
	...
	public void useSliceToLoadContent() {
		...
		Assertions.assertThat(users).containsAll(source.subList(5, 10));
	}
